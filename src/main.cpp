...
unsigned int nTargetSpacing = 2 * 60; // 60 seconds
unsigned int nRetarget = 30;
unsigned int nStakeMinAge = 10 * 60 * 60; // 10 hours
unsigned int nStakeMaxAge = 100 * 60 * 60; // 100 hours
...
const string strMessageMagic = "DXPCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // dxpcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // dxpcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nRewardCoinYear = POS_STAKE_REWARD; // 10% per year after 5th year

    if(nBestHeight < 262800)
        nRewardCoinYear = 12 * nRewardCoinYear; //120% reward for 262800 blocks 1st  year
    else if(nBestHeight < 525600)
        nRewardCoinYear = 6 * nRewardCoinYear; // 10% reward for 3153600 blocks 2nd year
    else if(nBestHeight < 788400)
        nRewardCoinYear = 3.6 * nRewardCoinYear; // 36% reward for 4730400 blocks 3rd Year
    else if(nBestHeight < 788400)
        nRewardCoinYear = 2.4 * nRewardCoinYear; // 24% reward for 4730400 blocks 4th Year
    else if(nBestHeight < 1051200)
        nRewardCoinYear = 1.2 * nRewardCoinYear; // 12% reward for 4730400 blocks 5th Year
...
// dxpcoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // dxpcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // dxpcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        // dxpcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // dxpcoin: track money supply and mint amount info
...
// dxpcoin: total coin age spent in transaction, in the unit of coin-days.
...
// dxpcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // dxpcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // dxpcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // dxpcoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // dxpcoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 10; //coinbase maturity
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // dxpcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // dxpcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // dxpcoin: getblocks may not obtain the ancestor block rejected
    ...
    // dxpcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DXPCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "DXP Era starts from 17 August 2018";
        CTransaction txNew;
        txNew.nTime = 1534498000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("ca5345762d5357c005dee1b1f49cdcd9fc6797e7533ec58fa8256bbb88bab154"));
        block.print();
        ...
        // dxpcoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xa9, 0x43, 0x57, 0xce };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // dxpcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // dxpcoin: tell downloading node about the latest block if it's
