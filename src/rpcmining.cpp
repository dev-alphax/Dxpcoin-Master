...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DXPCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DXPCoin is downloading blocks...");

    //int fPoW_Switch = GetArg("-powenable", 0);

    if (GetBoolArg("-testnet")){
        if (pindexBest->nHeight >= P1_End_TestNet){
            throw JSONRPCError(RPC_MISC_ERROR, "No more PoW blocks");
        }
    }else {
        if (pindexBest->nHeight >= P1_End){
            throw JSONRPCError(RPC_MISC_ERROR, "No more PoW blocks");
        }
    }
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DXPCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DXPCoin is downloading blocks...");
