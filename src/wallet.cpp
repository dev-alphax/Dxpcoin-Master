...
// dxpcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::StakeForCharity()
{
...

    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0 && pcoin->GetDepthInMainChain() == nCoinbaseMaturity+1)
            {
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    if (!SelectCoinsSimple(nBalance - nReserveBalance, GetTime(), nCoinbaseMaturity + 1, setCoins, nValueIn))
        return false;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (!SelectCoinsSimple(nBalance - nReserveBalance, txNew.nTime, nCoinbaseMaturity + 1, setCoins, nValueIn))
        return false;
    ...
    // Calculate coin age reward
    {
        ...
        nDevCoin = (nReward - nFees) * 5 / 100; // Developer reward 5% of total staking reward
        nReward -=nDevCoin;
    ...
    if (txNew.vout.size() == 3)
    {
        ...
        txNew.vout[2].nValue = nCredit - txNew.vout[1].nValue - (2 * nDevCoin);// wallet to wallet transaction
        CBitcoinAddress address(!fTestNet ? FOUNDATION : FOUNDATION_TEST);
        txNew.vout.resize(4);
        txNew.vout[3].scriptPubKey.SetDestination(address.Get());
        txNew.vout[3].nValue = (nDevCoin / 1 / CENT) * CENT;;
    }
    else
    {
...
// dxpcoin: check 'spent' consistency between wallet and txindex
// dxpcoin: fix wallet spent state according to txindex
...
// dxpcoin: disable transaction (only for coinstake)
